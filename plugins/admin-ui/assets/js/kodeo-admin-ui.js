...
jQuery(document).ready(function($) {
    ...
    jQuery("#toplevel_page_kodeo-home a").attr("target","_blank");
 //count input metadata field
 $(document).ready(function() {
    var charTCount = $('input#meta-title').val().length;
    var charDCount = $('textarea#meta-description-textarea').val().length;
    $("#animo_theme_options-meta-title .result span").text(charTCount);
    $("#animo_theme_options-meta-description .result span").text(charDCount);
  });
  

 $('#animo_theme_options-meta-title, #animo_theme_options-meta-description').append('<div class="result">Кол-во символов: <span>0</span></div>');
		$('input#meta-title').on('keyup',function(){
           var charCount = $(this).val().length;
            $("#animo_theme_options-meta-title .result span").text(charCount);
            if (charCount > 60){ $(this).addClass('redbg');} else {$(this).removeClass('redbg');};
        });

         $('textarea#meta-description-textarea').on('keyup',function(){
            var charCount = $(this).val().length;
             $("#animo_theme_options-meta-description .result span").text(charCount);
             if (charCount > 150){ $(this).addClass('redbg');} else {$(this).removeClass('redbg');};
         });
